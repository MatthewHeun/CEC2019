...
respectively),
leveraging a topology developed by Willis Jenkins (\textcite{Jenkins:2008}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ins{Discus the implications of views on the telos of Creation and eschatology, in addition to stewardship (all Christians have this) 
and how that impacts one's view of sustainability. We are not having the conversation about whether or not we need to worry 
about sustainability. Keywords: school of thought, worldview, perspective.}
...
Economics and politics (or ``the social'' aspects of sustainability) are inextricably linked. Biblical teachings on money
...
\ins{Perhaps because the Bible does not spell out a particular theory of economic and social theory, Christians through the ages
have advocated many different economic and social arrangements, from individualistic to collectivist. Use the same two words for 
economic and social with the ``economic'' and ``social'' modifiers.'}

\ins{All Christians understand that God has chosen to make us stewards of Creation. However, different Christian traditions have understood
that calling in different ways.} From the first chapter of Genesis, the call to stewardship of all creation has
...
One branch of Christian thought views wealth itself as a root of evil, which goes beyond merely \emph{love} money
...
communal economic arrangement, in imitation of the early church (Acts 2:42--46). Examples range from monastic orders such as
...
Thus, Christians hold a range of views from economic thought from  individualistic to communalistic.
...
%\ins{add 4-quadrant diagram?} what about a single axis?

\subsection{Christian solutions to the tragedy of the commons}
\label{sec:totc}

\ins{Link tragedy of the commons to sustainability.}
The term ``tragedy of the commons'' was popularized in (\cite{Hardin68}) % again no brackets here without a page number. cite twice again?!
and is a shorthand way of referring to situations where there is equal and open access to a resource or pool of
resources and it is in the rational self-interest of every individual to maximize their use of the resource, even if
it destroys the resource itself through over exploitation.  This section will examine several Christian responses to the tragedy of the commons.
...
The ``tragedy'' lies in the ``remorseless working out of things'' (\cite{Hardin68}). The tragedy of the commons is recognized as
belonging to a class of problems that ``[have] no technical solution,'' which Hardin defines as ``a change only in the techniques of 
the natural sciences, demanding little or nothing in the way of change in human values or ideas of morality.'' 
Instead, sustainable solutions to the tragedy of the commons come from social and/or economic policies.
...
only a limited number of animals on the common land. A collective solution would involve an authoritative governing
board administering the common land. The board decides how many animals total are allowed and what proportion
...
Christian \ins{economic and social} approaches to solving these problems.
...
Guided by Jenkins, one way to begin unpacking the four schools of thought 
...
of the stewardship school of thought for Reformed Christians doing creation care.
...
One consequence of that union is a radical, integral relationship between humans and 
...
is both crucially important \ins{as discussed above}
and entirely practical \ins{need to demonstrate this}. 
