...
We note that engineers 
%
\begin{enumerate*}[label={(\alph*)}]

  \item design and operate the machines and systems that
...
What guides policy-makers to make socio-economic-environmental
policy choices from which sustainability can emerge?

A traditional answer is ``the academic disciplines,'' 
which are both generators of and repositories of accumulated human knowledge
...
Recently, disciplines have emerged along the edges of Figure~\ref{fig:3_sustain}, too.
...
studies interactions between societies and their economies.
...
Our assessment of the vertex and edge disciplines is that they are not policy-relevant
at the macro level. 
These disciplines would focus on the best way to manage ``my forest'' 
rather than on the best way to preserve timber worldwide.
...
One might think that ``Christian Theology,'' one aspect of which is discerning the 
ways that deities interact with the natural world. 
However, there is little indication that theology 
